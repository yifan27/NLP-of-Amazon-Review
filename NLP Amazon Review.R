...
top_25k$myscore[i] <-sum(Matches[,1],na.rm = T)
next
...
top_25k$PN <- ifelse(top_25k$myscore>0,"P","N")
product_summary <- ddply(top_25k,c("ProductId"),summarize, N=length(Text), MeanRate = mean(Score),MeanRV = mean(myscore))
...
plot(top1$Score,top1$myscore)
abline(lm(top1$myscore~top1$Score))

top2 <- top_25k[which(top_25k$ProductId==product_top6[2,1]),]
plot(top2$Score,top2$myscore)
abline(lm(top2$myscore~top2$Score))

top3 <- top_25k[which(top_25k$ProductId==product_top6[3,1]),]
plot(top3$Score,top3$myscore)
abline(lm(top3$myscore~top3$Score))

top4 <- top_25k[which(top_25k$ProductId==product_top6[4,1]),]
plot(top4$Score,top4$myscore)
abline(lm(top4$myscore~top4$Score))

top5 <- top_25k[which(top_25k$ProductId==product_top6[5,1]),]
plot(top5$Score,top5$myscore)
abline(lm(top5$myscore~top5$Score))

top6 <- top_25k[which(top_25k$ProductId==product_top6[6,1]),]
plot(top6$Score,top6$myscore)
abline(lm(top6$myscore~top6$Score))
